...
public class EntityBorderRepelTask extends BukkitRunnable {
    ...
    public void run() {
        for (World world : Bukkit.getWorlds()) {
            for (LivingEntity entity : world.getLivingEntities()) {
                ...
                if (plot == null) {
                    // Check if mob is near border of a claim
                    Plot nearest = getNearbyPlot(loc);
                    if (nearest != null) {
                        // Push mob away from claim border
                        Vector push = loc.toVector().subtract(nearest.getChunk().getBlock(8, 64, 8).getLocation().toVector());
    ...
    private Plot getNearbyPlot(Location loc) {
        for (Plot plot : plotManager.getAllPlots()) {
            Location center = plot.getChunk().getBlock(8, 64, 8).getLocation();
            if (center.getWorld() != loc.getWorld()) continue;
